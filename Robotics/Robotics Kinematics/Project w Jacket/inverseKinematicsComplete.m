...
% read a csv file 
P_list_expected = table2array(readtable('Expected_2DoF.csv')); %positions = (x,y) in CSV file

%% TODO: Create a loop through the all the rows in P_list
[num_rowsP, num_colmsP] = size(P_list_expected);
P_list_predicted = zeros(num_rowsP, num_colmsP+1);
...
% ignore for now
        ...
        for j = 1:length(indexes) %it will loop only once for now
            ...
            for p = 1:num_rowsP %start reading P_list[]
                ...
                position_error      = 0.01 * unit_chosen;  % setting the accepted error to 1mm
                ...
                P_list_predicted(p,:) = [D_current ; i];
                
                
            end % loop through the points \
            
            varNames = {'x_p', 'y_p', 'iteration'};
            dataset_t = table(P_list_predicted(:,1),P_list_predicted(:,2), P_list_predicted(:,3), ...
                'VariableNames',varNames);

            writetable(dataset_t,strcat(num2str(alpha),'_numerical_ik_predictions.csv'),'Delimiter',',')
